...
function globalAskAI(action) {
    if (window.AIAssistant && typeof window.AIAssistant.requestAnalysis === 'function') {
        ...
        console.error('❌ AI助教實例未找到');
    }
}

// 測試衝突分析功能
function globalTestConflictAnalysis() {
    if (window.ConflictResolver && typeof window.ConflictResolver.testConflict === 'function') {
        window.ConflictResolver.testConflict();
    } else {
        console.error('❌ 衝突解決器未找到');
    }
}
